    ...
    with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                x = int(landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].x * image_width)
                y = int(landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].y * image_height)
                ...
                left_wrist_x = int(landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].x * image_width)
                left_wrist_y = int(landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].y * image_height)

                cv2.circle(image, (left_elbow_x, left_elbow_y), 10, (255, 255, 0), 5)
                cv2.circle(image, (left_shoulder_x, left_shoulder_y), 15, (255, 255, 0), 6)
                cv2.circle(image, (left_wrist_x, left_wrist_y), 15, (255, 255, 0), 6)

                cv2.line(image, (left_wrist_x, left_wrist_y), (left_elbow_x, left_elbow_y), (0, 0, 255), 4)
                cv2.line(image, (left_elbow_x, left_elbow_y), (left_shoulder_x, left_shoulder_y), (0, 0, 255), 4)

                # print(left_elbow)

                left_angle = int(calculate_angle(left_shoulder, left_elbow, left_wrist))

                if default_value == 'auto':


                    if left_angle > 90:
                        cv2.putText(image, 'Left Elbow Flexion',
                    ...
                    gotten_angles.append(left_angle)
                    if left_angle < 90:
                        cv2.putText(image, 'Left Elbow Extension',
                                    (0, 150),
                ...
                cv2.putText(image, f'Left Elbow Angle : {left_angle}', (0, 200), cv2.FONT_HERSHEY_DUPLEX, 1,
                            (0, 255, 255), 4)
                ...
                cv2.putText(image, 'LEFT ELBOW DETECTION ', (500, 100),
        ...
        with open(f'static2/left_elbow_angel@{file_id}.txt', 'w') as file:
