    ...
    with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                left_leg_ankel = [landmarks[mp_pose.PoseLandmark.LEFT_ANKLE].x,
                ...
                left_leg_ankel_x = int(landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].x * image_width)
                left_leg_ankel_y = int(landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].y * image_height)

                cv2.circle(image, (left_hib_x, left_hib_y), 10, (255, 255, 0), 5)
                cv2.circle(image, (left_knee_x, left_knee_y), 15, (255, 255, 0), 6)
                cv2.circle(image, (left_leg_ankel_x, left_leg_ankel_y), 15, (255, 255, 0), 6)

                cv2.line(image, (left_leg_ankel_x, left_leg_ankel_y), (left_knee_x, left_knee_y), (0, 0, 255), 4)
                cv2.line(image, (left_knee_x, left_knee_y), (left_hib_x, left_hib_y), (0, 0, 255), 4)
                ##########################################################

                left_knee_angle = int(calculate_angle(left_hib, left_knee, left_leg_ankel))

                if default_value == 'auto':

                    if left_knee_angle > 90:
                        cv2.putText(image, 'Left Knee Flexion',
                        ...
                        if left_knee_angle >= 90:
                            if direction == 1:
                                ...
                                direction = 0
                                gotten_angles.clear()
                    gotten_angles.append(left_knee_angle)
                    if left_knee_angle < 90:
                        cv2.putText(image, 'Left Knee Extension',
                                    (0, 150),
                ...
                cv2.putText(image, f'Left Knee Angle : {left_knee_angle}', (0, 200), cv2.FONT_HERSHEY_DUPLEX, 1,
                            (0, 255, 255), 4)
                ...
                cv2.putText(image, 'LEFT KNEE DETECTION ', (500, 100),
        ...
        with open(f'static2/left_knee_angel@{file_id}.txt', 'w') as file:
